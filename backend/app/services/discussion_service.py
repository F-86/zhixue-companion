...
def _require_discussion(discussion_id: str, course_id: str, db: Session) -> Discussion:
...
def create_discussion(course_id: str, teacher_id: str, title: str,
    ...
    return {
        ...
        "reply_count": 0, "created_by": _user_info(teacher), "created_at": d.created_at,
    }
...
def list_discussions(course_id: str, user_id: str, role: str,
    ...
    for d in discussions:
        ...
        items.append({
            ...
            "section_title": _section_title(d.section_id, db),
            "title": d.title, "status": d.status, "reply_count": reply_count,
            "created_by": _user_info(creator) if creator else {},
...
def get_discussion(course_id: str, discussion_id: str, user_id: str,
    ...
    if role == "teacher":
        ...
        _require_enrollment(course_id, user_id, db)
    d = _require_discussion(discussion_id, course_id, db)
    creator = db.get(User, d.created_by)
    ...
    for r in replies_raw:
        ...
        replies.append({
            ...
            "author": _user_info(author) if author else {},
            "is_teacher": (author.role == "teacher") if author else False,
            "created_at": r.created_at,
        })
    return {
        "id": d.id, "course_id": course_id,
        "section_id": d.section_id, "section_title": _section_title(d.section_id, db),
        "title": d.title, "content": d.content, "status": d.status,
...
def delete_reply(course_id: str, discussion_id: str, reply_id: str,
    ...
    _require_discussion(discussion_id, course_id, db)
