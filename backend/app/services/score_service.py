...
def get_student_course_scores(course_id: str, student_id: str, db: Session) -> dict:
    ...
    # 计算排名
    rank = _calc_rank(course_id, student_id, total_score, db)
    ...
    for e in enrollments:
        ...
        for a in assignments:
            ...
            if grade and grade.final_score is not None:
                ...
                scores.append(grade.final_score)
        total_score = round(sum(scores) / len(scores), 1) if scores else None
        rank = _calc_rank(e.course_id, student_id, total_score, db)
...
def get_course_score_distribution(course_id: str, teacher_id: str,
    ...
    for e in enrollments:
        ...
        if not student:
            continue
        from app.services.course_service import _calc_total_score
        total = _calc_total_score(course_id, e.student_id, db)
    ...
    for rank_idx, (student, total) in enumerate(student_scores, start=1):
        items.append({
            ...
            "total_score": total,
            "graded_assignments": _count_graded(course_id, student.id, db),
            "rank": rank_idx,
        })
    return {
        "course_id": course_id, "course_name": course.name if course else "",
...
def _calc_rank(course_id: str, student_id: str, my_score: float | None, db: Session) -> int | None:
    if my_score is None:
        return None
    from app.services.course_service import _calc_total_score
    enrollments = db.query(CourseEnrollment).filter(CourseEnrollment.course_id == course_id).all()
    ...
    for e in enrollments:
        if e.student_id == student_id:
            continue
        s = _calc_total_score(course_id, e.student_id, db)
        if s is not None and s > my_score:
            rank += 1
    return rank


def _count_graded(course_id: str, student_id: str, db: Session) -> int:
    assignments = db.query(Assignment).filter(Assignment.course_id == course_id).all()
    count = 0
    for a in assignments:
        sub = db.query(Submission).filter(
