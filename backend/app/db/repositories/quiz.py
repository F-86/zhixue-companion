...
def require_quiz(quiz_id: str, course_id: str, db: Session) -> Quiz:
...
def require_attempt(attempt_id: str, student_id: str, db: Session) -> QuizAttempt:
...
def is_correct(question: QuizQuestion, student_answer: str) -> bool:
    """判断客观题答案是否正确（不区分大小写）。"""
...
def serialize_question(q: QuizQuestion, include_answer: bool = False) -> dict:
    d = {
...
def create_quiz(course_id: str, teacher_id: str, title: str,
    ...
    db.commit()
    db.refresh(quiz)
    return get_quiz_detail(quiz, db, include_answer=True)
...
def update_quiz_status(course_id: str, teacher_id: str, quiz_id: str,
    ...
    require_teacher_course(course_id, teacher_id, db)
    quiz = require_quiz(quiz_id, course_id, db)
...
def get_quiz_attempts_summary(course_id: str, teacher_id: str, quiz_id: str, db: Session) -> dict:
    require_teacher_course(course_id, teacher_id, db)
    require_quiz(quiz_id, course_id, db)
...
def get_quiz_for_student(course_id: str, quiz_id: str, student_id: str, db: Session) -> dict:
    ...
    return {
        ...
        "questions": [serialize_question(q, include_answer=False) for q in questions],
        "attempt": {
...
def submit_attempt(course_id: str, quiz_id: str, attempt_id: str,
    ...
    require_enrollment(course_id, student_id, db)
    require_quiz(quiz_id, course_id, db)
    attempt = require_attempt(attempt_id, student_id, db)
    ...
    for q in questions.values():
        ...
        if q.question_type == "short_answer":
            try:
                ...
                is_correct_val = score_val >= q.score * 0.6
            except Exception:
                score_val = 0.0
                ai_feedback = "自动批改暂时不可用"
        else:
            is_correct_val = is_correct(q, student_answer)
            score_val = q.score if is_correct_val else 0.0
        total_score += score_val
        existing_ans = saved_answers.get(qid)
        if existing_ans:
            existing_ans.answer = student_answer
            existing_ans.is_correct = is_correct_val
            existing_ans.score = score_val
            existing_ans.ai_feedback = ai_feedback
        else:
            db.add(QuizAnswer(
                attempt_id=attempt_id, question_id=qid,
                answer=student_answer, is_correct=is_correct_val,
                score=score_val, ai_feedback=ai_feedback,
            ))
        results.append({
            "question_id": qid, "is_correct": is_correct_val,
            "score": score_val, "ai_feedback": ai_feedback,
            "correct_answer": q.correct_answer,
            "explanation": q.explanation,
        })
...
def save_answer(course_id: str, quiz_id: str, attempt_id: str,
    ...
    if existing_ans:
        ...
        db.add(QuizAnswer(attempt_id=attempt_id, question_id=question_id, answer=answer))
    db.commit()
...
def get_attempt_for_resume(course_id: str, quiz_id: str, attempt_id: str,
    ...
    if attempt.quiz_id != quiz_id:
        raise HTTPException(status_code=400, detail="attempt 不属于该测试")
    saved_answers = db.query(QuizAnswer).filter(QuizAnswer.attempt_id == attempt_id).all()
    ...
    return {
        ...
        "answers": answers_map, "answered_count": len(answers_map),
    }
...
def get_attempt_result(course_id: str, quiz_id: str, attempt_id: str,
    ...
    for q in questions:
        ...
        items.append({
            **serialize_question(q, include_answer=True),
...
def get_quiz_detail(quiz: Quiz, db: Session, include_answer: bool = False) -> dict:
    ...
    return {
        ...
        "questions": [serialize_question(q, include_answer=include_answer) for q in questions],
    }
