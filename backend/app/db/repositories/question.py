...
def require_question(question_id: str, course_id: str, db: Session) -> Question:
...
def can_see(q: Question, user_id: str, role: str) -> bool:
    """判断当前用户是否有权查看该问题"""
    if role == "teacher":
        return True
...
def list_questions(course_id: str, user_id: str, role: str,
    ...
    for question in questions:
        if not can_see(question, user_id, role):
            continue
        ...
        items.append({
            ...
            "section_title": section_title(question.section_id, db),
...
def get_question(course_id: str, question_id: str, user_id: str, role: str, db: Session) -> dict:
    if role == "teacher":
        ...
        require_enrollment(course_id, user_id, db)
    q = require_question(question_id, course_id, db)
    if not can_see(q, user_id, role):
    ...
    return {
        "id": q.id, "course_id": course_id,
        "section_id": q.section_id, "section_title": section_title(q.section_id, db),
