...
def require_teacher_course(course_id: str, teacher_id: str, db: Session) -> Course:
...
def create_course(teacher_id: str, name: str, description: str | None,
    ...
    course = Course(
        teacher_id=teacher_id, name=name, description=description,
        semester=semester, cover_image_url=cover_image_url,
...
def get_teacher_course(course_id: str, teacher_id: str, db: Session) -> dict:
    c = require_teacher_course(course_id, teacher_id, db)
    from app.models.section import Section
    student_count = db.query(CourseEnrollment).filter(CourseEnrollment.course_id == c.id).count()
    section_count = db.query(Section).filter(Section.course_id == c.id).count()
    return {
...
def list_course_students(course_id: str, teacher_id: str, db: Session) -> dict:
    ...
    for e in enrollments:
        ...
        if not student:
            continue
        total_score = calc_total_score(course_id, e.student_id, db)
        items.append({
...
def list_student_courses(student_id: str, status: str | None, db: Session) -> dict:
    ...
    for enrollment, course in rows:
        ...
        total_score = calc_total_score(course.id, student_id, db)
        items.append({
...
def get_student_course(course_id: str, student_id: str, db: Session) -> dict:
    ...
    total_score = calc_total_score(course_id, student_id, db)
...
def calc_total_score(course_id: str, student_id: str, db: Session) -> float | None:
...
def get_course_name(course_id: str, db: Session) -> str:
    c = db.get(Course, course_id)
    return c.name if c else ""


def count_enrollments(course_id: str, db: Session) -> int:
    return db.query(CourseEnrollment).filter(CourseEnrollment.course_id == course_id).count()
